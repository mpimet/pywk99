...
def plot_spectrum(spectrum: xr.DataArray,
    ...
    if ax is None:
        ax = plt.gca()
    if flagged_windows:
        new_spectrum = modify_spectrum(
            new_spectrum, flagged_windows, 'substract'
            )
    if use_log:
        with np.errstate(divide='ignore'):
            new_spectrum = np.log10(new_spectrum)
    new_spectrum.sel(
...
def plot_coherence(cross_spectrum: xr.DataArray,
                   ...
                   **kwargs) -> None:
    """Plot a longitude-time Power Spectrum."""
    new_spectrum = cross_spectrum.copy()
    if ax is None:
        ax = plt.gca()
    if flagged_windows:
        new_spectrum = modify_spectrum(
    ...
    coh2 = new_spectrum.coherence_squared
...
def plot_phase_arrows(cross_spectrum: xr.DataArray,
    ...
    coh2 = new_spectrum.coherence_squared
    coh2 = coh2.where(coh2 > coherence_threshold, drop=True)
    u = xr.apply_ufunc(np.real, 1j * new_spectrum.cross )
    u = u / np.abs(new_spectrum.cross)
    v = xr.apply_ufunc(np.imag, 1j * new_spectrum.cross )
    v = v / np.abs(new_spectrum.cross)
